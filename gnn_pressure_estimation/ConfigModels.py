...
from gnn_pressure_estimation.GraphModels import (
    ...
    GAT,
    GIN,
)  # , GATResRewrittenConv, GATResLinearConv


def config_gatres_large(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    args.model_path = (
        r"experiments_logs\simple_test\GATResMeanConvLarge_znorm_25b_128c_20235401_20230601_1754\best_GATResMeanConvLarge_znorm_25b_128c_20235401.pth"
    )
    args.criterion = "mse"
    ...
    return args, GATResMeanConv(
        name="GATRes_Large_znorm_25b_128c" if test_model_variant_name is None else test_model_variant_name, num_blocks=25, nc=128
    )
...
def config_gatres_small(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.norm_type = "znorm"
    return args, GATResMeanConv(
        name="GATResMeanConv_small_znorm_15b_32c" if test_model_variant_name is None else test_model_variant_name, num_blocks=15, nc=32
    )
...
def config_gat(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.norm_type = "znorm"
    return args, GAT(
        name="GAT_10b_32c_2h" if test_model_variant_name is None else test_model_variant_name, num_blocks=10, nc=32, in_channels=1, out_channels=1
    )
...
def config_gcn2(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.norm_type = "znorm"
    return args, GCN2(
        name="GCN2-64b-32nc" if test_model_variant_name is None else test_model_variant_name, num_blocks=64, nc=32, in_channels=1, out_channels=1
    )
...
def config_gatres_small_tough(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.norm_type = "znorm"
    return args, GATResMeanConv(
        name="GATResMeanConv_small_tough_znorm_15b_32c" if test_model_variant_name is None else test_model_variant_name, num_blocks=15, nc=32
    )


def select_model(
    args: argparse.Namespace, test_model_variant_name: str = None, reset_model_path: bool = False
