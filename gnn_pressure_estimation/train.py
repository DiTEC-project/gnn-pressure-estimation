...
def train_one_epoch(
    ...
    grad_clipper: GradientClipping = None,
) -> tuple[float, dict, dict, Any]:
    """train an epoch
...
def internal_train(
    args: argparse.Namespace, model: torch.nn.Module, train_ds: WDNDataset, val_ds: WDNDataset, test_ds: Optional[WDNDataset], do_load: bool = True
) -> tuple[float, dict, dict]:
    ...
    for epoch in range(1, args.epochs + 1):
        tr_loss, tr_metric_dict, tr_record_metric_dict, out = train_one_epoch(
            model=model,
            optimizer=optimizer,
            loader=train_loader,  # num_nodes=num_nodes,
        ...
        val_loss, val_metric_dict, val_record_metric_dict = test_one_epoch(
            model=model,
            loader=val_loader,  # num_nodes=num_nodes,
            mask_rate=args.mask_rate,
            device=device,
            mean=mean,
            std=std,
    ...
    if args.do_test:
        ...
        return evaluation.internal_test(args=testing_args, model=trained_model, train_ds=train_ds, test_ds=test_ds, do_load=False)
    else:
...
def train(args: argparse.Namespace, model: torch.nn.Module = None, do_load=True):
    ...
    return internal_train(args=args, model=model, train_ds=train_ds, val_ds=val_ds, test_ds=test_ds, do_load=do_load)
...
def get_arguments(raw_args):
    ...
    parser.add_argument(
        ...
        default=False,
        type=bool,
        help="after training, we evaluate the model on clean or noisy tests. However, we should evaluate a different pipeline. As such, this flag is set to False by default.",
    )
    ...
    parser.add_argument(
        "--patience", default=100, type=int, help="Early stopping patience in these epochs. If val_loss unchanges, the training is stopped"
    )
    ...
    parser.add_argument(
        ...
        choices=["keep_all", "keep_list", "keep_junction", "reservoir", "tank"],
        type=str,
        help="simple-keep_all, tough-keep_list. Node removal strategy to remove different nodal types in the water network. Support: keep_list| reservoir| tank| keep_junction| keep_all",
    )
    parser.add_argument(
        "--device",
        default="cuda",
    ...
    parser.add_argument(
        ...
        default=10.0,
        type=float,
        help="percentile from historical gradients used for gradient clipping. Only used when use_gradient_clipping is True",
    )
    parser.add_argument(
        "--scheduler",
        default=None,
        ...
        help="scheduler name. Support ReduceLROnPlateau. Set None if unused",
    )  #
    ...
    parser.add_argument(
        "--norm_type",
        default="znorm",
        choices=["znorm", "minmax", "unused"],
        type=str,
        help="normalization type. Support znorm| minmax|unused or None",
    )
