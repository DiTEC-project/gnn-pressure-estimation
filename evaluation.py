...
def test_one_epoch(model:torch.nn.Module,
                    ...
                    use_same_mask:bool=False,) -> tuple[float, dict]:
    ...
    if do_test_on_sensors:
        required_idx, required_uid = get_sensors(test_input_path= test_input_path,
                                                    feature= feature,
    ...
    with torch.no_grad():
        for data in loader:
            ...
            if (use_same_mask and not created_all_mask) or not use_same_mask:
                ...
                all_mask = generate_batch_mask(num_nodes=num_nodes,
                                                mask_rate=mask_rate,
...
def test_clean(model:torch.nn.Module,
                test_ds: WDNDataset,
                ...
                criterion:Callable, 
                metric_fn_dict:dict, 
                edge_attrs:list[str], ) -> tuple[list, dict, list, dict]:
    test_losses=[]
...
def test_noisyNN(model:torch.nn.Module,
                test_ds: WDNDataset,
                args: argparse.Namespace,
                device:str, 
                mean:Any,  
...
def test_noisy11(model:torch.nn.Module,
                test_ds: WDNDataset,
                args: argparse.Namespace,
                device:str, 
                mean:Any,  
...
def test_and_collect_once(model:torch.nn.Module,
                    loader:DataLoader,
                    trial:int,
                    args: argparse.Namespace,
                    device:str, 
                    ...
                    criterion:Callable, 
                    metric_fn_dict:dict, 
                    edge_attrs:list[str], ) -> tuple[float, dict, float, dict]:
    #for all nodes
    test_loss, test_metric_dict = test_one_epoch(model=model,
                                                    loader=loader,
    ...
    test_sensor_loss, test_sensor_metric_dict = test_one_epoch(model=model,
                                                    loader=loader,
...
def internal_test(args: argparse.Namespace, model: torch.nn.Module, train_ds: WDNDataset, test_ds: WDNDataset, do_load: bool=True) -> tuple[dict, dict, dict]:
    """perform a full test
    ...
    print_multitest_metrics(trials=trials,
...
def test(args: argparse.Namespace, model: torch.nn.Module =None, do_load=True) -> tuple[dict,dict,dict]:
    ...
    return internal_test(args=args,
                        model=model,
                        train_ds=train_ds,
                        test_ds=test_ds,
                        do_load=do_load)
...
def get_arguments(raw_args):
        ...
        parser.add_argument('--train_val_removal',default='keep_all', type=str, help="simple-keep_all, tough-keep_list. Node removal strategy. Support: keep_list| reservoir| tank| keep_junction| keep_all")
        ...
        parser.add_argument('--test_type',default='clean', type=str,  choices=['clean','noisy11','noisyNN'],  help="test strategy. clean- test with a clean scenario and repeat N times | noisy11- test N times with a pair of noise scene and random mask | noisyNN- test N noise scenes in which each is tested with N masks, so total is NxN cases ")
